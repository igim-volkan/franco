...
const Dashboard: React.FC<DashboardProps> = ({ customers, opportunities, events, globalTasks, onViewChange }) => {
  ...
  const salesData = [
    { name: 'Teklif Verildi', value: opportunities.filter(o => o.status === OpportunityStatus.PROPOSAL_SENT).length },
    { name: 'Teklif Görüşüldü', value: opportunities.filter(o => o.status === OpportunityStatus.PROPOSAL_DISCUSSED).length },
    { name: 'Detaylandırıldı', value: opportunities.filter(o => o.status === OpportunityStatus.PROPOSAL_DETAILED).length },
    { name: 'Kapanışa Yakın', value: opportunities.filter(o => o.status === OpportunityStatus.CLOSE_TO_CLOSING).length },
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 group">
            <div className="flex items-start justify-between mb-6">
              ...
              <div className={`flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest ${stat.trend === 'up' ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'
                }`}>
